...
def tau(p0):
    ...
    if 'log_xch4' in parameters:
        ...
        for i, t in enumerate(temperature_array):
            ...
            integrand_methane_grid = np.zeros((len(pressure_array_pmin), opacity_line_length))  # This will be the integrand for water

            for j, p in enumerate(pressure_array_pmin):

                p = p/1e6   # Need bars to read in opacity file
                opacity, x_full = load_opacity(t, p)  # load methane opacity for this temperature and pressure
                print(opacity)
            ...
            for j, p in enumerate(pressure_array_pmin):
                ...
                integral_value = np.trapz(integrand_methane_grid_sliced, pressure_sliced,
                                      axis=0)  # calculate integral using trapezoid approximation
    ...
    if 'log_xco' in parameters:
        ...
        for i, t in enumerate(temperature_array):
            ...
            integrand_carbon_monoxide_grid = np.zeros((len(pressure_array_pmin), opacity_line_length))  # This will be the integrand for water

            for j, p in enumerate(pressure_array_pmin):
 
                p = p/1e6   # Need bars to read in opacity file
                opacity, x_full = load_opacity(t, p)  # load carbon monoxide opacity for this temperature and pressure
                print(opacity)
            ...
            for j, p in enumerate(pressure_array_pmin):
                ...
                integral_value = np.trapz(integrand_carbon_monoxide_grid_sliced, pressure_sliced,
                                      axis=0)  # calculate integral using trapezoid approximation
    ...
    for molecule in molecules:
        ...
        if 'log_xh2o' in parameters:
            integral_dict[molecular_name] = integral_water_grid
        if 'log_xch4' in parameters:
            integral_dict[molecular_name] = integral_methane_grid
        if 'log_xco' in parameters:
            integral_dict[molecular_name] = integral_carbon_monoxide_grid
